...
2. Требование ТЗ к расширяемости комманд (_"Необходимо предусмотреть простую реализацию расширения списка команд, например, настроить частоту опроса или что-нибудь еще."_) осложняется за счет полного отсутствия единообразия команд. Расширение комманд возможно на стороне сервера в файле ```RequestHandler.cpp``` за счет добавления указателей на функции в соответсвующий контейнер:
```
